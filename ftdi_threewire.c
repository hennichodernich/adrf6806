...
void threewire_writeraw24(t_spipintriple spipins, uint32_t data)
{
    ...
    for (bitctr = 0; bitctr < 24;bitctr++)
    {
        data_bit=(data_copy & 0x00800000) ? 1 : 0;
        //digitalWrite(spipins.dio, (data_copy & 0x00800000));
        buf[0] = (data_bit << spipins.dio); // all low, except data pin, if 1
        f = ftdi_write_data(ftdi, buf, 1);        
        if (f < 0) fprintf(stderr,"write failed\n");
        usleep(1);
        ...
        data_copy = (data_copy << 1) & 0x00FFFFFF;
    }
    usleep(1);
